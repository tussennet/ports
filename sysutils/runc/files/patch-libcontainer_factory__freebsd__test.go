--- libcontainer/factory_freebsd_test.go.orig	2020-10-02 12:15:25 UTC
+++ libcontainer/factory_freebsd_test.go
@@ -0,0 +1,152 @@
+// +build freebsd
+
...
+	}
+	lfactory, ok := factory.(*FreeBSDFactory)
+	if !ok {
+		t.Fatal("expected FreeBSD factory returned on linux based systems")
+	}
...
+	}
+	fcontainer, ok := container.(*freebsdContainer)
+	if !ok {
+		t.Fatal("expected freebsd container on freebsd based systems")
+	}
+	if fcontainer.initProcessPid != expectedState.InitProcessPid {
+		t.Fatalf("expected init pid %d but received %d", expectedState.InitProcessPid, fcontainer.initProcessPid)
+	}
+}
+
